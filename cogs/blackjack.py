...
class Blackjack(commands.Cog):
    ...
    async def send_blackjack(self, startup=False):
        ...
        if goal_system_dict["enabled"] and self.bot.gain_or_lose > goal_system_dict["amount"]:
            if not self.gamble_flags["goal_reached"]:
                ...
                await self.bot.log(f"goal reached - {self.bot.gain_or_lose}/{goal_system_dict['amount']}, stopping blackjack!", "#4a270c")
                notify(f"goal reached - {self.bot.gain_or_lose}/{goal_system_dict['amount']}, stopping blackjack!", "blackjack - Goal reached")
        ...
        if amount_to_gamble > self.bot.user_status["balance"] and not self.bot.settings_dict["cashCheck"]:
            if not self.gamble_flags["no_balance"]:
                ...
                await self.bot.log(f"Amount to gamle next ({amount_to_gamble}) exceeds bot balance ({self.bot.user_status["balance"]}), stopping blackjack!", "#4a270c")
                notify(f"Amount to gamle next ({amount_to_gamble}) exceeds bot balance ({self.bot.user_status["balance"]}), stopping blackjack!", "blackjack - Insufficient balance")

            await self.bot.sleep_till(self.bot.settings_dict["defaultCooldowns"]["moderateCooldown"])
            return await self.send_blackjack()
        elif self.gamble_flags["no_balance"]:
            await self.bot.log(f"Balance regained! ({self.bot.user_status["balance"]}) - restarting blackjack!", "#4a270c")
        ...
        if (self.bot.gain_or_lose + (self.bot.settings_dict["gamble"]["allottedAmount"] - amount_to_gamble) <=0):
            if not self.gamble_flags["amount_exceeded"]:
                ...
                await self.bot.log(f"Allotted value ({self.bot.settings_dict["gamble"]["allottedAmount"]}) exceeded, stopping blackjack!", "#4a270c")
                notify(f"Alloted value ({self.bot.settings_dict["gamble"]["allottedAmount"]}) exceeded, stopping blackjack!", "blackjack - Alloted value exceeded")
        ...
        if amount_to_gamble > 250000:
            await self.bot.log(f"Value to gamble ({amount_to_gamble}) exceeded 250k threshhold, stopping blackjack!", "#4a270c")
            notify(f"Value to gamble ({amount_to_gamble}) exceeded 250k threshhold, stopping blackjack!", "blackjack - Exceeded 250k limit")
    ...
    async def on_message(self, message):
        if message.channel.id == self.bot.cm.id and message.author.id == self.bot.owo_bot_id:
            if message.embeds:
                if message.embeds[0].author.name is None or self.bot.user.name not in message.embeds[0].author.name:
                    return
                for embed in message.embeds:
                    if embed.footer and any(item in embed.footer.text for item in ["🎲 ~ game in progress", "🎲 ~ resuming previous game"]):
                        ...
                        optimal_move = find_optimal_move(res["dealer"], res["our"], res["soft"])
                        await self.click_reaction(message, optimal_move)
                        

    @commands.Cog.listener()
    async def on_message_edit(self, before, after):
        if before.channel.id == self.bot.cm.id and before.author.id == self.bot.owo_bot_id:
            if after.embeds:
                if after.embeds[0].author.name is None or self.bot.user.name not in after.embeds[0].author.name:
                    return
                for embed in after.embeds:
                    if embed.footer:
                        if "🎲 ~ game in progress" in embed.footer.text:
                            ...
                            self.game_event.set()
                            lost_amt = int(re.search(r"([\d,]+)", embed.footer.text).group(0).replace(',', ''))

                            await self.bot.update_cash(lost_amt, reduce=True)
                            self.bot.gain_or_lose-=lost_amt
                            self.turns_lost+=1

                            await self.bot.log(f"lost {lost_amt} in bj, net profit - {self.bot.gain_or_lose}", "#993f3f")
                            await self.send_blackjack()
                            ...
                            self.game_event.set()
                            self.turns_lost = 0
                            win_amt = int(re.search(r"([\d,]+)", embed.footer.text).group(0).replace(',', ''))
                            ...
                            self.game_event.set()
                            await self.bot.log("didn't win or lose blackjack..", "#ffafaf")
                            await self.send_blackjack()
